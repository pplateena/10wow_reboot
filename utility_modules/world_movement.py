...
def mover(queue,stop_flag,cp_list):
    ...
    for checkpoint in cp_list:
        ...
        while True:
            ...
            if magnitude > req_dist and abs(delta) < 60:
...
def decision_maker(going_todo=None, location= None):
    ...
    if type(going_todo[-1]) is str:
        extra_action = going_todo[-1]
        going_todo.pop()
        priority = 0
    elif type(going_todo[0]) is str:
        extra_action = going_todo[0]
        going_todo.pop(0)
        priority = 1
    else:
        priority = 0
    ...
    if going_todo[-1] == (84.57,78.80,0.15,255):
        cici.press_key('w', 3)
